...
resource "oci_identity_dynamic_group" "FunctionsServiceDynamicGroup" {
  provider = oci.homeregion
  name = "baggage-tracking_demo"
  description = "baggage-tracking_demo. Created by rpalve."
  compartment_id = var.tenancy_ocid
...
resource "oci_identity_policy" "FunctionsServiceDynamicGroupPolicy" {
  ...
  provider = oci.homeregion
  name = "baggage-tracking_demo_faas"
  description = "baggage-tracking_demo_faas Created by rpalve."
...
#  ,"allow any-user to {STREAM_READ, STREAM_CONSUME} in compartment id ${var.compartment_ocid} where all {request.principal.type='serviceconnector',  target.stream.id='oci_streaming_stream.export_baggage-tracking_demo_3.id',  request.principal.compartment.id='${var.compartment_ocid}'}"
  ]
